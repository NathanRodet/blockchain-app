...
describe("PrivilegeCard", function () {
  ...
  describe("Card Management", function () {
    ...
    it("Should allow an admin to create a card", async function () {
      await privilegeCard.createCard(1, "Gold", ethers.parseEther("1"), 75, 10, "http://0nlyF@n-MYM.com/HERE-COMES-THE-MONEY.png", "Discount Card Description");
    ...
    it("Should throw a quantity exception when quantity is not at least 1", async function () {
      await expect(privilegeCard.createCard(0, "Invalid Card", ethers.parseEther("1"), 50, 0, "http://0nlyF@n-MYM.com/HERE-COMES-THE-MONEY.png", "Invalid Card Description")).to.be.revertedWith("Quantity must be at least 1");
    });

    it("Should throw an exception when non-owner tries to transfer a card", async function () {
      await privilegeCard.createCard(1, "Gold", ethers.parseEther("1"), 75, 10, "http://0nlyF@n-MYM.com/HERE-COMES-THE-MONEY.png", "Gold Card Description");
      await privilegeCard.connect(addr1).buyCard(1, { value: ethers.parseEther("1") });
  ...
  describe("PrivilegeCard Ownership Features", function () {
    beforeEach(async function () {
      await privilegeCard.addAdmin(await owner.getAddress());
      await privilegeCard.createCard(1, "Gold", ethers.parseEther("1"), 75, 100, "http://0nlyF@n-MYM.com/gold.png", "Gold Card Description");
    });
    ...
    it("Should correctly track multiple card purchases by the same user", async function () {
      await privilegeCard.createCard(2, "Silver", ethers.parseEther("0.5"), 50, 10, "http://0nlyF@n-MYM.com/silver.png", "Silver Card Description");
      await privilegeCard.createCard(3, "Bronze", ethers.parseEther("0.2"), 25, 10, "http://0nlyF@n-MYM.com/bronze.png", "Bronze Card Description");
